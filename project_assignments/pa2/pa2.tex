...
The reference and measured positions of \texttt{joint\_3} were recorded in a CSV file and then plotted in MATLAB against elapsed time.

\begin{lstlisting}[style=ros, language=Python,
    caption={CSV logging setup in \texttt{controller\_node.py}}]
...
For every control cycle, the elapsed time, reference position, and measured position are recorded in separate rows.

\begin{lstlisting}[style=ros, language=Python,
    caption={Logging a control cycle in \texttt{control\_loop}}]
t_elapsed = (now - self.start_time).nanoseconds * 1e-9
self.log_writer.writerow([t_elapsed, self.ref_position, self.cur_position])
self.log_file.flush()
\end{lstlisting}

A clean shutdown is ensured with an additional \texttt{destroy\_node}:

\begin{lstlisting}[style=ros, language=Python,
    caption={Clean shutdown}]
def destroy_node(self):
    self.log_file.close()
    super().destroy_node()
\end{lstlisting}

The MATLAB script reads the CSV file, extracts the time, reference position, and measured position data from it, then plots the reference position and measured position against time.

\begin{lstlisting}[style=ros, language=Matlab,
    caption={\texttt{plot\_joint3.m}}]
...
The resulting figure is shown in Figure~\ref{fig:matlab}.
